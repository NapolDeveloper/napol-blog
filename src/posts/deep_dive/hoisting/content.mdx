...
여기서 필자는 한 단계 더 나아가 보려고 한다. 자바스크립트를 해석하고 실행하는 `v8` 엔진에서는 `var`, `let`, `const` 키워드를 이용할 때 어떤 방식으로 처리하는지 알아보자.
...
다음으로 `kVar` 모드라면 선언과 동시에 `undefined` 로 초기화를 해주어야 하므로 메모리를 할당해야 하낟. 그래서 `kVar` 의 경우 메모리를 할당하는 코드를 찾아보았지만, 클론받은 패키지에서는 찾지 못했다..
